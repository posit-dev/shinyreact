...
> **Note:** Uses `shinyreact`. For the `ui.tsx`-first equivalent that uses *actual* shadcn components (real `cva`, real Tailwind v4, real `Card`/`Button`/etc.), see [04-shadcn](../../ui-tsx/04-shadcn/).
...
```bash
uv run shiny run examples/app-py/05-shadcn/app.py
```

## When to prefer this vs. 04-shadcn
...
- **[04-shadcn](../../ui-tsx/04-shadcn/):** real shadcn components with `cva` variants, real Tailwind v4, lucide icons. Brings a Vite + npm setup but gives you the actual shadcn registry and TypeScript-friendly variants.
