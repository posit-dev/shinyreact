...
> **Note:** Uses `shinyreact`. `client-ui`-first apps generally don't need a per-output-type bridge — the client picks how to render whatever JSON the server ships. See [example 16](../16-shadcn/) for matplotlib + plotly versions of the plot pattern.
