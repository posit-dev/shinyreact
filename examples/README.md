...
| Example | Description |
|---------|-------------|
| [01-hello](01-hello/) | Smallest `ui.tsx` app — server with reactive logic only, plus a static React client (no JSX, no bundler). Side-by-side comparison of client-only state vs. server-routed state to highlight websocket latency. Includes both `app.py` and `app.R` servers over the same `www/` client |
...
| [07-plotly](07-plotly/) | Embeds `@render_plotly` via `ShinyOutput` and `set_react_page()` |
