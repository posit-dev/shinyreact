...
[07-plotly](07-plotly/) and [11-npm-bare](11-npm-bare/) also ship an `app.R`
...
(a testthat drift guard fails otherwise). 11-npm-bare is not copied: its
...
| [11-npm-bare](11-npm-bare/) | The npm tier with nothing else on the page: the client imports `@posit-dev/shinyreact` from npm, as 09-hmr does, and the server is `page_bare(page_react_dep(...))` — no shinyreact JS, no `#shinyreact-config` tag, no protocol handshake. `app.py` + `app.R` |
...
Most of the Python tests drive the app itself, with shiny's built-in
[`local_server`](https://shiny.posit.co/py/api/testing/) pytest fixture — an
already-started `test_server()` session: inputs in, output values out, in
memory. That is a good fit for these apps
...
def test_dist_caption(local_server):
