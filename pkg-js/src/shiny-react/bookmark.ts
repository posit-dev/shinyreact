...
export function applyRestoredValues(registry: InputRegistry): void {
  ...
  // version checking.
  //
  // A missing tag is not an error in either build: a `page_bare()` page
  // legitimately has none, and there is nothing to check when the server
  // serves no client at all (#261).
