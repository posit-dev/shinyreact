...
// --- shinyreact bridge ---
function ShinyNavigationMenu({ element }) {
  const { input_id, items = [], className } = element.props;
  const [, _setValue] = useShinyInput(input_id ?? "__noop_nav__", null, { debounceMs: 0, priority: "event" });
  const setValue = input_id ? _setValue : null;

  function handleClick(e, href, label) {
    if (setValue) {
      e.preventDefault();
      setValue({ value: label, nonce: Math.random() });
    }
  }
                  ...
                  {item.items.map((sub, j) => (
                    <li key={j}>
                      <NavigationMenuLink asChild>
                        <a
                          href={sub.href ?? "#"}
                          onClick={(e) => handleClick(e, sub.href, sub.label)}
...
export { ShinyNavigationMenu as NavigationMenu };
