...
// --- shinyreact bridge ---
function MenubarItems({ items, onSelect }) {
  return items.map((item, i) => {
    switch (item.type) {
      case "label":
        return <MenubarLabel key={i}>{item.label}</MenubarLabel>;
      case "separator":
      ...
      case "submenu":
        return (
          <MenubarSub key={i}>
            ...
            <MenubarSubContent>
              <MenubarItems items={item.items ?? []} onSelect={onSelect} />
            </MenubarSubContent>
          </MenubarSub>
        );
      case "item":
...
function ShinyMenubar({ element }) {
  const { input_id, menus = [], className } = element.props;
  const [, setValue] = useShinyInput(input_id, null, { debounceMs: 0, priority: "event" });
  function handleSelect(menuLabel, itemValue) {
    setValue({ menu: menuLabel, value: itemValue, nonce: Math.random() });
  }
  return (
    <Menubar className={className}>
      {menus.map((menu, i) => (
        <MenubarMenu key={i}>
          ...
          <MenubarContent>
            <MenubarItems items={menu.items ?? []} onSelect={(v) => handleSelect(menu.label, v)} />
          </MenubarContent>
        </MenubarMenu>
      ))}
    </Menubar>
...
export { ShinyMenubar as Menubar };
