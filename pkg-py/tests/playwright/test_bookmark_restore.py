...
EXAMPLE_APP = (
    ...
    / "examples/app-py/13-bookmarking/app.py"
)

app = create_app_fixture(EXAMPLE_APP)
...
def test_url_mode_restores_inputs(page: Page, app: ShinyAppProc) -> None:
...
def test_no_bookmark_renders_defaults(page: Page, app: ShinyAppProc) -> None:
    ...
    # Default values from bookmarking.js: txt="", num=0, chk=false.
    expect(page.locator("input[type=text]")).to_have_value("")
    expect(page.locator("input[type=number]")).to_have_value("0")
...
def test_server_mode_round_trip(page: Page, tmp_path: Path) -> None:
