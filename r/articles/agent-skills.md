...
shinyreact ships two [Agent Skills](https://agentskills.io/) so a coding
agent can build a shinyreact app without you pasting documentation into
it:

- **`shinyreact-build-app`** builds a `ui.tsx`-pattern app from scratch.
- **`shinyreact-convert-app`** ports an existing Shiny app to the
  pattern. It inspects the source, drives the running app in a browser,
  describes it in plain English, then rewrites it against that
  description.
