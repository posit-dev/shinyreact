...
- **Static `Node` in page chrome** (e.g. `page_react(tags.div(Node("Chart", …)))`, no render function feeding it) → `tagify()` emits a self-contained mount carrying its spec as a child script:
  ...
  **Post-load insertion — resolved in [#120](https://github.com/posit-dev/shinyreact/issues/120).** This design originally seeded `.shinyreact-static` mounts only once, at `DOMContentLoaded` (a YAGNI deferral). A mount inserted *after* load — `Node.tagify()` via `insertUI`, or a `@render.ui` returning a `Node` — went unseeded and rendered nothing. **Decision: keep static mounts distinct from outputs (no convergence with `output_ui`), and seed post-load insertions with a standing `MutationObserver`.** Convergence was rejected because an output requires an id and a WebSocket value, neither of which an inline self-contained static mount has — routing them through the output binding would mean a synthetic id and a phantom output registration. A non-monkeypatch Shiny hook was ruled out (the `@posit/shiny` public surface offers only id-required input/output binding registries, `renderContent`/`bindAll`, and `addCustomMessageHandler`). DOM-event listeners were also evaluated and rejected: `shiny:bound` fires on the *binding element*, and a `.shinyreact-static` mount is not a binding — in the canonical `@render.ui`→`Node` case the inserted content has no binding, so `shiny:bound` never fires for it. `shiny:value` does fire on the output element, but it is cancelable and fires *before* the default render, so the mount is not yet in the DOM at event time (deferral is racy with async dependency loading), and it never fires for `insertUI()` of binding-free content. The `MutationObserver` reacts to the actual DOM mutation, so it is correctly timed by construction and uniform across `@render.ui`, `insertUI`, and even non-Shiny insertions. It was chosen over wrapping `Shiny.bindAll` to avoid coupling an infrastructure package to another library's internals; `seedMount`'s `hasRoot` guard keeps it idempotent, and `seedInlineSpecs(root?)` is now scoped so the observer seeds only freshly-inserted subtrees. `seedInlineSpecs` is also exposed on `window.shinyreact` as a manual escape hatch.
