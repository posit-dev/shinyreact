...
Known issues and open work items. See `features.md` and `features-shinyjson-old.md` for what already exists.
...
Currently `ui_output`, `page_react`, and `page_bare` are flat top-level exports (in `shinyjsonold`). Later, restructure into a `shinyjson.ui` submodule: `ui.output()`, `ui.page_react()`, `ui.page_bare()`.
