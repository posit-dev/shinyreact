...
- for apps that own a complete HTML document (what a Vite build emits)
  ...
  - the file is read at call time
    ...
    - `[r]` no cache at all: read twice on **every** call — once with `brio` for
      the marker check, once by `htmlTemplate()` to render
...
- the discovered UI is a **per-request function**, so `bookmark_store="url"`
  ...
  - `www/` is mounted at `/` whenever the directory exists, since
    ...
    404 per asset
    - so the mount is latched at construction even though the mode is not: if
      `www/` does not exist when `ReactApp()` runs, creating it mid-session
      switches the mode but serves no assets until a restart
