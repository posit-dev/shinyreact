...
3. **Use the prototype to evaluate the role of the JSON spec transfer layer** (`Spec`, `Element`, `Node`, originally rendered via `@json-render/react` and now via an in-house walker — see #46) instead of assuming that question is already settled.
