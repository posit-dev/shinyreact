...
- `shinyreact.page_react_html(path="www/index.html")` — Core-mode helper for apps that own a complete HTML document with a `{{ headContent() }}` marker (the kind a Vite build emits); Shiny's and shinyreact's tags render at the marker. Pass the result to `shinyreact.App` (not `shiny.App` — upstream gap py-shiny#2462), which also auto-mounts the document's directory. Does not auto-discover renderer dependencies. Does not support `bookmark_store` (rendered once at startup; use `page_react()` in a `def app_ui(request)` function for bookmarked apps)
...
- `page_react_html(path = "www/index.html")` matches Python: a complete HTML document with a `{{ headContent() }}` marker. R uses it directly as `shinyApp(ui = ...)`; Python needs the `shinyreact.App` wrapper (py-shiny#2462). Python additionally has the Express-only `set_react_page()`.
