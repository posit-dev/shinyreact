...
- `shinyreact.page_react(src_dir=None, js_file="ui.js", css_file="ui.css", version=None, title=None, theme=None)` — Core-mode zero-config page: discovers `www/ui.js` / `www/ui.css` next to the calling module, serves them as an mtime-versioned dependency (cache-busted), title defaults to the app folder name; the client appends its own mount container to `<body>`
...
- `shinyreact.page_bare(*args, title=None, lang="en", theme=None)` / `shinyreact.page_react_dep(*, src_dir=None, js_file="ui.js", css_file="ui.css", name=None, version=None)` — escape-hatch page builder and app-bundle `HTMLDependency` helper. `version` defaults to `js_file`'s mtime (dev cache-busting); pass a fixed one when publishing
